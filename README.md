...
| Per-reply cost chip | `conversation.chat.assistant-actions` | The anchored cost of one finalized reply as a single `Pill` capsule, closing the Turn's action row after its timing text and coloured by the band that priced it (green off-peak, red peak); the slot hands over the message id, which the `sessionCostIndex` projection resolves to ledger coordinates, and the band plus its multiplier ride the title (dash `—` when unpriced) |
